...
function hasVowels(string) {
    return string.includes('a') || string.includes('A') || string.includes('e') || string.includes('E') || string.includes('i') || string.includes('I') || string.includes('o') || string.includes('O') || string.includes('u') || string.includes('U')
};
...
function startsAndEndsWithVowel(string) {
    return isVowel(string[0]) && isVowel(string.charAt(string.length - 1))
};
...
function getSmallestNumber(seq){
    return Math.min.apply(null, seq);
}
...
function hasEvens(nums){
    const isEven = nums.some(numbers => numbers % 2 === 0);
    return isEven;
}
...
function hasOdds(nums){
    const isOdd = nums.some(numbers => numbers % 2 !== 0);
    return isOdd;
}
...
function onlyPositiveEvens(nums){
    const filterNums = nums.filter(numbers => numbers % 2 === 0 && numbers > 0);
    return filterNums;
}
...
function onlyPositiveOdds(nums){
    const filterOdds = nums.filter(numbers => numbers % 2 !== 0 && numbers > 0);
    return filterOdds;
}
...
function onlyNegativeEvens(nums){
    const filterNegEvens = nums.filter(numbers => numbers % 2 === 0 && numbers < 0);
    return filterNegEvens;
}
...
function onlyNegativeOdds(nums){
    const filterNegOdds = nums.filter(numbers => numbers % 2 !== 0 && numbers < 0);
    return filterNegOdds;
}
...
function shortestString(arr){
    const reduceShortestString = arr.reduce((a,b) => a.length <= b.length ? a : b);
    return reduceShortestString;
}
...
function longestString(arr){
    const reduceLongestString = arr.reduce((a,b) => a.length >= b.length ? a : b);
    return reduceLongestString;
}
...
function elementsTimesTwo(numArr){
    const timesTwo = numArr.map(numbers => numbers * 2);
    return timesTwo;
}
